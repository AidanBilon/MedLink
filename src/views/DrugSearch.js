...
export default function DrugSearch() {
  ...
  // Accept overrides so clicks/changes take effect immediately
  const fetchPage = async (targetPage = 0, limitOverride, queryOverride) => {
    const effectiveLimit = Number(limitOverride ?? limit) || 10;
    const effectiveQuery = (queryOverride ?? query) || "";
    const effectiveSkip = targetPage * effectiveLimit;

    setLoading(true);
    setError("");
    try {
      const endpoint = buildEndpoint(effectiveQuery, effectiveLimit, effectiveSkip);
  ...
  const handleSearch = async (e) => {
    e.preventDefault();
    await fetchPage(0); // uses current state
  };

  // When limit changes, immediately refetch using the current query
  const onChangeLimit = async (e) => {
    const newLimit = Math.max(1, Math.min(50, Number(e.target.value) || 10));
    setLimit(newLimit);
    setPage(0);
    await fetchPage(0, newLimit); // apply immediately
  };

  // NEW: quick-select runner (sets query AND fetches immediately)
  const runQuickSearch = async (term) => {
    setQuery(term);
    setPage(0);
    await fetchPage(0, undefined, term);
  };
  ...
  // Full-bleed background (edge-to-edge), repeats vertically, flush under top bar
  ...
  const examples = ["Amoxicillin", "Ibuprofen", "Acetaminophen", "Metformin"];

  return (
    <div style={bgWrapperStyle}>
      <Container style={{ maxWidth: 920, marginTop: 0 }}>
        ...
        <Form onSubmit={handleSearch} style={{ marginBottom: 12 }}>
          ...
          {/* QUICK SUGGESTIONS — clicking runs search immediately */}
          ...
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                ...
                <Button
                  color="light"
                  disabled={(skip + limit) >= (meta?.results?.total || 0) || loading}
        ...
        {/* Empty state (kept, but quick suggestions above now also auto-search) */}
        {!loading && !error && results.length === 0 && (
          <div style={{ border: "1px dashed #e5e7eb", borderRadius: 16, padding: 20, background: "#fafafa" }}>
            <p className="mb-2">
              <strong>No results.</strong> Try a broader term or an advanced query like{" "}
              <code>purpose:"pain relief"</code> or <code>drug_interactions:caffeine</code>.
            </p>
